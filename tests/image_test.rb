...
class TestImage < Test::Unit::TestCase
  ...
  def test_initialization_successful
    ...
    assert_equal(expected_img_grid, im.imageGrid)
  ...
  def test_get_pixel
    ...
    assert_equal(im.imageGrid[1][0], im.get_pixel(2, 1))
    assert_equal(im.imageGrid[1][1], im.get_pixel(2, 2))
    assert_equal(im.imageGrid[1][2], im.get_pixel(2, 3))
    assert_equal(im.imageGrid[1][3], im.get_pixel(2, 4))
    assert_equal(im.imageGrid[1][4], im.get_pixel(2, 5))
  end
  ...
  def test_set_pixel_successful
    im = Image.new(2, 5)
    row = col = 1
    assert_equal('O', im.get_pixel(row, col))

    # Color pixel
    im.set_pixel(row, col,'A')
    assert_equal('A', im.get_pixel(row, col))
  end

  def test_set_pixel_throws_on_out_of_bounds
    im = Image.new(2, 5)
    assert_raise(IndexError) do
      im.set_pixel(3, 4, 'A')
    end
  ...
  def test_set_pixel_throws_on_invalid_color_string
    im = Image.new(2, 5)
